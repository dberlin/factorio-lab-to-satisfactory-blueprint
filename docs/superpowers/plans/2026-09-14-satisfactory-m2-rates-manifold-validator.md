...
4. **Attachment turns instead of arcs (user ruling, R2 superseded).** Wherever the corridor laying turns a belt (corridor to row, row to corridor, entry and exit), place a `Build_ConveyorAttachmentSplitter_C` (or merger, whichever direction of flow it is) with exactly one input and one output linked, yawed so the through port and one side port make the 90°; the belt legs into and out of it are straight and at least the minimum length. The turn's footprint is the attachment's 400 cm soft box, which belts may pass through; two of them back to back are a 180° in 4 m of depth instead of the arc's 8 m. `quarter_turn` remains for the one case an attachment cannot serve (state it, or delete the builder if none remains). The validator already accepts an attachment with unlinked ports; `ports.direction` and `flow.capacity` treat the through pair like any splitter.
