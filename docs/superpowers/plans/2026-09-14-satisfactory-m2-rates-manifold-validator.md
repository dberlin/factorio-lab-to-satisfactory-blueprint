...
- **R2 (superseded 2026-09-15 by a user ruling).** Originally: belts only, lifts in M3, every turn a belt arc. The user ruled that turns use the game's zero-footprint idioms: a splitter or merger with one input and one output connected is a 90° turn in 2 m; two lifts make a U-turn or a reversal (belt into the first lift's bottom, up, its top faces the new direction, into the second lift's top, down, out its bottom), and a lift may go straight into a port with its top facing any of the four directions. These are options beside the belt arc, not a mandate (user, 2026-09-15 03:10 UTC): at each turn the laying code picks whichever of arc, attachment turn or lift pair costs the least space where it stands, and a lift is charged for the column of space it blocks, since lifts get worse as a blueprint fills. Passthroughs and the stacking contract stay M3. Task 8d adds the attachment turn and the choice; Task 8e adds the lift pair as the third option.
...
4. **Attachment turns as a choice beside arcs (user ruling, R2 superseded).** Wherever the corridor laying turns a belt (corridor to row, row to corridor, entry and exit), it may place a `Build_ConveyorAttachmentSplitter_C` (or merger, whichever direction of flow it is) with exactly one input and one output linked, yawed so the through port and one side port make the 90°; the belt legs into and out of it are straight and at least the minimum length. The turn's footprint is the attachment's 400 cm soft box, which belts may pass through; two of them back to back are a 180° in 4 m of depth instead of the arc's 8 m. `quarter_turn` stays a live primitive: a pure function chooses per turn between the arc and the attachment by the space each costs where it stands, the choice is recorded in the placement description, and both outcomes are tested. The validator already accepts an attachment with unlinked ports; `ports.direction` and `flow.capacity` treat the through pair like any splitter.

- [ ] **Step 1: Failing tests**: `iron-plate-60` lays out clean in **Mk1** with `power.wires` run and the strict corpus pin updated; the depth of that build is asserted below 3200 cm with the number; the direct-pairing test asserts three straight belts and no merger/splitter between the rows; a non-matching pair (`iron-plate-50`: 2.5 constructors) still gets a manifold; the feeder length equals 101 cm; a corridor turn is an attachment with one input and one output linked and two straight legs, and the validator passes it.
...
### Task 8e (added 2026-09-15, user ruling): lifts as turns

**Files:** `tools/sfy-native` (disasm runs only), `scripts/sfy_native_rules.py` and `hologram_rules.json` (two `compute` rules), `scripts/sfy_registry.py`/`registry.py` (lift connector geometry with sources), `src/flab2bp/sfy/layout/model.py` (`LiftObj`), `emit.py`, `validate.py`, `laying.py`/`corridors.py`, `docs/sfy-layout-model.md`, tests.

1. **Extract the lift's connector geometry from the game.** A lift's two ports are at the actor origin with no rotation in the registry, so where its bottom and top connections sit, which way each faces, and how the top's yaw is chosen, is set at runtime. Read it from the headers (`Buildables/FGBuildableConveyorLift.h`: `mTopTransform`, `mIsReversed`, `mMeshHeight`, the connection members) and the binary (`AFGBuildableConveyorLift::SetupConnections` / `BeginPlay` / whatever positions `mConnection0`/`mConnection1` from `mTopTransform`, and `AFGConveyorLiftHologram::UpdateTopTransform` which the `lift.height_range` rule already quotes) as `lift.connectors` and `lift.top_yaw` `compute` rules with evidence, then into the registry (`Buildable.lift: LiftGeometry(bottom_offset, bottom_facing, top_offset_fn, top_yaw_free: bool, reversed_swaps_flow: bool)` with sources). Which end items enter by when `mIsReversed` is set comes from `Factory_Tick` the way the belt flow did. A fixture lift is a format example of the `mTopTransform` tag shape only.
2. **Model, emit, decode, validate.** `LiftObj(id, class_name, pose, height_cm, top_yaw_deg, reversed)`; `emit` writes the lift with its `mTopTransform` (and `mIsReversed`) from the geometry rule; `decode` reads it back (the M2 `decode` error for lifts goes away); validator: `lift.height` (ours, stricter: within `lift_min_cm`/`lift_max_cm`, and `lift_min_vertical_cm` when attached to a port; `lift.height_range` is a clamp, so the refusal is ours and says so), `lift.step` (ours, multiples of `lift_step_cm`), `lift.placement` (`refuse`, extracted: implement its comparison), `ports.position` extended to lift ends, `belt.capsule` gets the lift's clearance from `lift.clearance` (partial: the read part, skip-noted).
3. **Use them.** In corridor laying: a U-turn or reversal is two lifts; a level change (a bridge over a crossed column) is a lift up, a straight belt across, a lift down, instead of inclines; a lift may end straight in a machine or attachment port. The corridor gap and wall margins shrink accordingly; the strategy prefers the attachment turn, then the lift pair, then an arc.
4. **Tests and gate**: synthetic placements for each primitive validate clean; `decode(emit(p)) == p` with lifts; the strict audit re-pinned; checkpoint 2 re-emitted; the plan's R2 note points here.
