...
- [ ] **Step 2: Tier leaf.** Failing test in `tests/sfy/test_corpus.py`: extend `test_importing_the_satisfactory_corpus_adds_no_dsp_module_of_its_own` so that importing `flab2bp.bench.sfy_corpus` in a fresh interpreter loads **no** `flab2bp.dsp.*` and no `flab2bp.rates` module at all (today it is "no more than `bench.corpus` pulls", which is nine). Move `Tier` into `bench/tier.py` importing nothing but `enum`; `bench/corpus.py` imports it from there; `sfy_corpus.py` imports the leaf. Run: `uv run pytest tests/sfy/test_corpus.py -q; echo exit=$?` → 0. Commit: `Move Tier into a leaf so the Satisfactory corpus imports no DSP module`.
