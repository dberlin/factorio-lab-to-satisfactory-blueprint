...
- **R-M3-2. Occupancy is the legal minimum on the lattice, and every place it is stricter is named.** Belts go everywhere the game's boxes let them: beside machines at port height, between machines, over them, under bridges. A node at level `k` is impassable for belts when the 158 × 158 × 30 cm box centred on it (twice `BELT_CLEARANCE_HALF_WIDTH_CM` square, twice `BELT_CLEARANCE_HALF_HEIGHT_CM` tall, orientation-free) meets any hard clearance box, the designer wall, or a lift's column box. A committed straight run blocks its own nodes and the two nodes across the run from each of them at its level for every other net: two belts 100 cm apart overlap by 58 cm, and a belt ending or turning on the node beside a run reaches 50 cm along its own last segment into the run's 79 cm, so both are collisions the validator would report, not simplifications. Adjacent levels never interact (belt boxes are 30 cm tall, levels 100 cm apart). Where the lattice IS stricter than the game, and by how much: (a) belt pitch is 200 cm because 158 cm is not a lattice multiple, 42 cm per belt; (b) the node beyond a run's last node along the run stays free for a perpendicular belt (their boxes clear by 21 cm), and only that node; (c) R7's one grid step between machine hard boxes stays, because `buildable.clearance` is `partial` and touching boxes may not be assumed legal; (d) a belt's own path from its port to its machine's box edge lies INSIDE that machine's hard box (a Constructor's input is 200 cm inside it), so those nodes are opened for that port's net alone, the way the validator excuses a belt from the box its own port sits in, and no other net may use them. Each of the four is recorded in `docs/sfy-layout-model.md`, and the evidence reports belt pitch and belt count per cell so the cost of (a) is measured rather than assumed.
- **R-M3-3. Levels 0 and 1 are impassable everywhere** (the slab, and the band between the slab and the ports). **Level 2 is the port level and the ground belt level**: every belt port of a machine standing on the slab is at z 200, so the densest belts, the ones that run straight into and out of ports and alongside the machines, are all at level 2. Levels above cost a toll of `0.01 × (k − 2)` per step so the router prefers the port level and climbs only to cross. Legal lift heights are `4 ≤ h ≤ min(48, n − 2)` levels (`lift_min_cm`, `lift_max_cm`, `lift_step_cm`, read).
...
- [ ] **Step 1: Failing tests.** `test_node_and_world_are_inverses_on_every_mark`; `test_a_constructor_blocks_levels_one_to_seven_and_four_lines_out` (centre on a node: nodes with `|dx| ≤ 4` and `|dy| ≤ 5` at levels 1..7 blocked, level 8 free, line 5 in x free); `test_a_committed_run_shadows_the_two_nodes_across_it_and_not_the_one_beyond_its_end`; `test_two_belts_two_nodes_apart_are_both_free_and_validate_clean` (pitch 200 is legal: the boxes clear by 42 cm); `test_a_belt_from_a_constructor_port_crosses_its_own_box_and_no_other_nets` (R-M3-2 (d)); `test_a_belt_runs_beside_a_machine_at_port_height_one_line_outside_its_box`; `test_rip_up_restores_the_base_not_one`; `test_flags_agree_with_free_on_every_node` (the invariant, over a placement with two machines, one belt, one lift and one splitter: `all(occ.flags[occ.lattice.index(node)] == occ.free(node) for node in every node)`); `test_a_lift_column_blocks_its_levels_between_the_ends`; `test_levels_zero_and_one_are_never_passable`.
