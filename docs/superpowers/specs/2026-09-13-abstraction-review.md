...
**Status 2026-09-13: done, by Plan B** (`docs/superpowers/plans/2026-09-13-abstraction-repairs-2-plan-b.md`, branch `abstraction-repairs-2b`, eight tasks). `flab2bp/layout/budget.py` now owns the whole contract: `expired(deadline, clock)` is the one predicate, `WorkBudget` is the ledger the `budget["left"]` dict used to be, `StagedWorkBudget` is the former `ExpansionBudget`, and `BudgetExhausted` is the base of every "ran out" signal. The five named predicates (`routing_domain._expired`, `compact_seed._deadline_reached`, `finalize._completion_expired`, `routing_proposals.check_deadline`, `hierarchy/compose._spent`) and the three closures survive by name -- tests and scripts patch them -- as one-line delegations. `routing_domain._routing_pass_budget` is the only place a pass-sized ledger is seeded, so the three callers can no longer disagree about the `_ROUTING_BUDGET` floor. `tests/test_one_deadline_rule.py` is the durable guard: no clock-versus-bound comparison outside `budget.py`, no `{"left": ...}` ledger, no `WorkBudget(left=...)` outside the factory and five listed carve sites, no read of the floor outside the factory.

Three counts in the paragraphs above were verified wrong while executing, and are left as written for the record: there were **five** named predicates, not four (`hierarchy/compose._spent` is the fifth); **eight** inline comparisons, not the 33 R2 claims; and **42** dict sites, not 35. There is also **no runtime rewrite of `_ROUTING_BUDGET`** in the tree: `routing_domain.py` has exactly one assignment, the definition, and what `freeform` did was a cross-module late-bound *read* through the module object while `sequence_solver` and `hierarchy/compose` bound the same constant at import time. That inconsistency is what the factory removed; nothing was deleted that had to be un-written first.

Three deliberate departures from the proposal above, each evidenced. (1) The three exception types became **subclasses of one `BudgetExhausted` base**, not one class with an optional payload: `_route_all` converts a caught `_PreparationDeadline` into the proposals `Deadline` (`routing_domain.py:8213-8214`) exactly so that the enclosing `except _GeometricDeadline` returns `None` instead of letting the preparation deadline propagate. One class makes that conversion a no-op and changes which handler fires. (2) The deadline stays an **explicit parameter** inside `routing_domain` rather than folded into the budget object, because `_route_all` rebinds its `deadline` nonlocal mid-run (`finally: deadline = route_deadline`) and derives per-query deadlines; folding a rebound name into a shared object changes which clock a later check reads. (3) `_geometric_search` still takes **15 parameters**; item 10 of section 9 predicted it shrinks "once P1 and P4 land", and P4 (the canvas and grid representations) was not in this plan.
