...
The concrete/mk1 factory converts a measured manifold depth refusal. The
plate/mk1 factory compares against checkpoint 2. Neither naturally needs lifts;
...
CASES = (
    _case("concrete-60", "does the smallest converted factory deliver 60 concrete/min"),
    _case("iron-plate-60", "does grid routing deliver the same 60 plates/min as checkpoint 2"),
)
...
def instructions(factories: tuple[Written, ...], witness: TransportWitness) -> str:
    out = [
        ...
        "FactorioLab flows and the production `grid-routed` strategy without overrides.",
        "Concrete/mk1 converts a measured manifold depth refusal; plate/mk1 is the",
        "like-for-like comparison with checkpoint 2. Both are naturally flat.",
        "",
        ...
        "3. Connect the listed -Y inputs and +Y outputs. Power the two factories;",
        "   the standalone conveyor witness needs no power.",
...
def build_checkpoint(
    ...
    try:
        pipeline.build(
            ...
            flow=FLOWS / converted.flow,
            time_budget_s=time_budget_s,
        )
    except NoValidLayout as refused:
        if refused.reason != "rows exceed the designer depth":
            raise CheckFailed(
                f"the claimed manifold depth conversion changed: {refused.reason}"
            ) from refused
    else:
        raise CheckFailed("concrete/mk1 now builds under the manifold; update the conversion case")
    factories = tuple(
        build_case(case, out_dir, time_budget_s=time_budget_s, strategy="grid-routed")
        for case in CASES
    )
